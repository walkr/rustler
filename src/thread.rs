...
pub trait AsyncSend {
    ...
    ///
    fn spawn<'a, F>(env: NifEnv<'a>, thread_fn: F)
    ...
    fn spawn_job<F: FnOnce() + Send + panic::UnwindSafe + 'static>(job: F);
}

pub struct ThreadAsyncSend;
impl AsyncSend for ThreadAsyncSend {
    fn spawn_job<F: FnOnce() + Send + panic::UnwindSafe + 'static>(job: F) {
        thread::spawn(job);
    }
}
