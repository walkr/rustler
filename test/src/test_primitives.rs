...
pub fn add_u32<'a>(env: &'a CallerEnv, args: &Vec<NifTerm>) -> NifResult<NifTerm<'a>> {
    let lhs: u32 = try!(args[0].decode());
    let rhs: u32 = try!(args[1].decode());

    Ok((lhs + rhs).encode(env))
}
pub fn add_i32<'a>(env: &'a CallerEnv, args: &Vec<NifTerm>) -> NifResult<NifTerm<'a>> {
...
pub fn tuple_add<'a>(env: &'a CallerEnv, args: &Vec<NifTerm>) -> NifResult<NifTerm<'a>> {
    let tuple: AddTuple = try!(args[0].decode());
    Ok((tuple.lhs + tuple.rhs).encode(env))
}

pub fn echo_u8<'a>(env: &'a CallerEnv, args: &Vec<NifTerm>) -> NifResult<NifTerm<'a>> {
